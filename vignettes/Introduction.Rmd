...
p_dat <-  fit %>% 
  # first get unreported used by index i
  tidybayes::spread_draws(p[i]) %>%
  # now join with regions and times data
  dplyr::left_join(
    dplyr::mutate(d,i = dplyr::row_number()),
    by="i"
  ) %>%
  dplyr::group_by(region_name,times) %>%
  dplyr::summarise(
  ...
  p_use_plot <- p_dat %>% ggplot2::ggplot(ggplot2::aes(x=times)) +
      ggplot2::geom_ribbon(
